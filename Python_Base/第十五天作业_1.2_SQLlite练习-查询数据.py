...
while True:
    ...
    if user_input == '0':
        ...
        if not user_sn:
            continue
        print(search_result_str("select * from qytang_homework_info where 姓名 = '{0}'".format(user_sn)))
    elif user_input == '3':
        user_age = input('搜索大于输入年龄的学员，请输入学员年龄：')
        if user_age == '':
            continue
        print(search_result_str("select * from qytang_homework_info where 年龄 > {0}".format(user_age)))
    elif user_input == '4':
        user_homework = input('搜索大于输入作业数的学员，请输入学作业数量：')
        if user_homework == '':
            continue
        print(search_result_str("select * from qytang_homework_info where 作业数 > {0}".format(user_homework)))
    else:
        print('输入错误！请重新输入！')
